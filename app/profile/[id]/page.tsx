...
export default function ProfilePage({
	...
	const isLoggedIn = true; // 임시로 true 처리
	...
	useEffect(() => {
		...
		async function fetchExplanation() {
			try {
				const response = await axios.post("http://localhost:8000/explain_individual", {
					query,
					numericId,
				});
	...
	if (query && loading) {
		return (
			<div className="min-h-screen bg-[#121317] text-white">
				<nav className="sticky top-0 z-10 bg-[#121317]/90 backdrop-blur-sm border-b border-[#1c1f2a] px-3 py-2">
					<div className="max-w-4xl mx-auto flex items-center justify-between">
						<div className="h-8 w-24 bg-[#1c1f2a] rounded-md animate-pulse" />
						<div className="h-8 w-8 bg-[#1c1f2a] rounded-full animate-pulse" />
					</div>
				</nav>
				...
				<main className="p-8">
					<div className="max-w-3xl mx-auto">
						<div className="flex items-start gap-6 mb-8">
							<div className="w-24 h-24 bg-[#1c1f2a] rounded-full animate-pulse" />
							<div className="flex-1 space-y-4">
								<div className="space-y-2">
									<div className="h-8 w-48 bg-[#1c1f2a] rounded-md animate-pulse" />
									<div className="h-5 w-72 bg-[#1c1f2a] rounded-md animate-pulse" />
								</div>
								<div className="space-y-3">
									<div className="h-5 w-56 bg-[#1c1f2a] rounded-md animate-pulse" />
									<div className="h-5 w-48 bg-[#1c1f2a] rounded-md animate-pulse" />
									<div className="h-5 w-64 bg-[#1c1f2a] rounded-md animate-pulse" />
								</div>
							</div>
						</div>

						<div className="space-y-6">
							<div className="h-12 bg-[#1c1f2a] rounded-md animate-pulse" />
							<div className="space-y-4">
								<div className="h-32 bg-[#1c1f2a] rounded-lg animate-pulse" />
								<div className="h-24 bg-[#1c1f2a] rounded-lg animate-pulse" />
								<div className="h-28 bg-[#1c1f2a] rounded-lg animate-pulse" />
							</div>
						</div>
					</div>
				</main>
	...
	const backUrl = query ? `/search?query=${query}` : "/";

	return (
		<div className="min-h-screen bg-[#121317] text-white">
			<nav className="sticky top-0 z-10 bg-[#121317]/90 border-b border-[#1c1f2a] px-3 py-2">
				<div className="max-w-4xl mx-auto flex items-center justify-between">
					<a href={backUrl} className="flex items-center text-[#A8A8A8] hover:text-white">
						<svg
			...
			<main className="p-8">
				<div className="max-w-3xl mx-auto">
					...
					<div className="mb-8">
						<div className="flex items-start justify-between">
							<div className="flex gap-6">
								<div className="w-24 h-24 bg-[#1c1f2a] rounded-full flex items-center justify-center overflow-hidden">
									{userImageUrl ? (
										<Image
											src={userImageUrl}
					...
					{/* Tabs */}
					<Tabs defaultValue={query ? "explanation" : "info"} className="w-full">
						<TabsList className="w-full bg-[#1D1F2A] border-b border-[#252836] flex">
							{query && (
								<TabsTrigger
									value="explanation"
									className="flex-1 rounded-none data-[state=active]:bg-[#1D1F2A] text-gray-300 data-[state=active]:text-white data-[state=active]:border-b-2 data-[state=active]:border-[#A980F6] border-b-2 border-transparent"
								>
									추천사유
								</TabsTrigger>
							)}
						...
						{query && (
							<TabsContent value="explanation" className="mt-6">
								<div className="bg-gradient-to-r from-[#E4E5F9] from-2% via-[#A980F6] via-30% to-[#CB97FC] p-0.5 rounded-lg">
									<div className="bg-transparent p-4 rounded-lg">
										<h3 className="text-black font-semibold mb-2">추천 사유</h3>
										<p className="text-black text-sm">{explanation}</p>
									</div>
								</div>
							</TabsContent>
						)}
							...
							{transformedUserData.projects.length > 0 && (
								<div className="space-y-4">
									{transformedUserData.projects.map((project, index) => (
										<div key={index} className="bg-[#1c1f2a] p-4 rounded-lg">
									...
									<div className="bg-[#1c1f2a] p-4 rounded-lg">
										...
										<div className="flex flex-wrap gap-2">
											{transformedUserData.skills.main.map((skill, index) => (
												<span key={index} className="px-3 py-1.5 bg-[#252836] rounded-full text-xs">
													{skill}
												</span>
											))}
										</div>
